...
router=({path,component,transition,props})=>{let ts=transition&&{style:{transition:`all ${transition.timeout}ms`},timeout:transition.timeout,In:transition.in,timeout:transition.timeout,transition:transition.className};return cv.c("router",ts,cv.c(component,props))},
